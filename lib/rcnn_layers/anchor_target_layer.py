...
def anchor_target_layer_2d(height, width, gt_boxes, input_shape, _feat_stride, all_anchors, num_base_anchors):
  ...
  A = total_anchors / num_base_anchors
  ...
  print('anchor height range: {:g} => {:g}'.format(all_anchors[:,3].min(),all_anchors[:,3].max()))
  print('anchor width range: {:g} => {:g}'.format(all_anchors[:,2].min(),all_anchors[:,2].max()))
  ...
  # labels
  labels = labels.reshape((1, height, width, K)).transpose(0, 3, 1, 2)
  labels = labels.reshape((1, 1, K * height, width))
  rpn_labels = labels
